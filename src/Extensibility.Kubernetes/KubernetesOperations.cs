...
{
    public class KubernetesOperations : IResourceOperations
    {
        ...
        private (KubernetesConfig config, KubernetesClientConfiguration clientConfig) InitializeConfig(Import import)
        {
        ...
        private static (string name, string? @namespace) GetKeys(Resource resource)
        {
            if (resource.Properties!.TryGetValue("metadata", out var token) && token is JObject metadata)
            {
